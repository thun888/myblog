...
...
  ...
  el += partial('dateinfo')
  //字数显示
  el += '<div class="flex-row" id="page-words"><span style="padding: 4px;">本文字数：' + page.content.length + '字</span><span class="sep updated" style="padding: 4px;"></span><span class="text updated" style="padding: 4px;">阅读时长：' + parseInt(page.content.length/250) + '分</span></div>';
```
