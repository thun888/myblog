...
{% hashtag Geyser https://d.hzchu.top/libmr %}
...
> 使用`sudo pip install mysql-connector-python `安装依赖

![表结构](https://onep.hzchu.top/mount/pic/myself/2024/07/66961677a8d46.png?fmt=avif)

```mysql
CREATE TABLE `mcserve`.`chat`  (
  `id` int NOT NULL,
...
{% quot el:h3 prefix:https://fclouds.hzchu.top/assets/favicon.ico 在线服务 %}
...
​	这就有点意思了，为了实现这些数据统计，我在游戏中新建了`若干的计分板`+一些在常加载区块里的`命令方块`，随后每隔一段时间读取存档里的存储着计分板数据的文件（\world\data\scoreboard.dat），将数值计入数据库。（起初使用Rcon执行命令显示该玩家所有计分板数据并使用正则表达式提取再存入，前期还好，到后来对性能影响太大被迫改进）
