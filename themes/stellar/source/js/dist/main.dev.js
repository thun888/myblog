...
var l_body = document.querySelector('.l_body');
var sidebar = {
...
var init = {
  ...
  relativeDate: function relativeDate(selector) {
    selector.forEach(function (item) {
      var $this = item;
      var timeVal = $this.getAttribute('datetime');
      var relativeValue = util.diffDate(timeVal, true);
  ...
  registerTabsTag: function registerTabsTag() {
    ...
    document.querySelectorAll('.tabs ul.nav-tabs .tab').forEach(function (element) {
      element.addEventListener('click', function (event) {
        event.preventDefault(); // Prevent selected tab to select again.

        if (element.classList.contains('active')) return; // Add & Remove active class on `nav-tabs` & `tab-content`.

        _toConsumableArray(element.parentNode.children).forEach(function (target) {
...
init.registerTabsTag(); // scrollreveal
...
if (stellar.plugins.lazyload) {
  stellar.loadScript(stellar.plugins.lazyload.js, {
    defer: true
...
if (stellar.plugins.sitesjs) {
  var issues_api = document.getElementById('sites-api');
...
if (stellar.plugins.friendsjs) {
  var _issues_api = document.getElementById('friends-api');

  if (_issues_api != undefined) {
    stellar.jQuery(function () {
...
if (stellar.plugins.swiper) {
  var swiper_api = document.getElementById('swiper-api');
...
if (stellar.plugins.fancybox) {
  var selector = 'img[fancybox]:not(.error)';

  if (stellar.plugins.fancybox.selector) {
    selector += ", ".concat(stellar.plugins.fancybox.selector);
  }

  if (document.querySelectorAll(selector).length !== 0) {
    stellar.loadCSS(stellar.plugins.fancybox.css);
    stellar.loadScript(stellar.plugins.fancybox.js, {
      defer: true
    }).then(function () {
...
if (stellar.plugins.heti) {
  ...
  stellar.loadScript(stellar.plugins.heti.js, {
    defer: true
  }).then(function () {
    var heti = new Heti('.heti p');
    heti.autoSpacing();
