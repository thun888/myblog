...
hexo.extend.helper.register('popular_posts_wrapper', function (args) {
  ...
  var json = args.json.json;
  var cls = args.json["class"];

  if (json == undefined || json.length == 0) {
    return '';
  }

  var cfg = hexo.theme.config.article.related_posts;
  if (cfg.enable != true) return;
  var returnHTML = "";
  var div = "\n    <section class='header'>\n      <div class='title cap theme'>".concat(title, "</div>\n    </section>\n    <section class='body'>\n    ");
  ...
  function generateHTML(list) {
    ...
    if (p) {
      if (p.cover) {
        ...
        var params = '';
        p.tags.reverse().forEach(function (tag, i) {
          if (i > 0) {
            params += ',';
          }
    ...
    if (list.excerpt && list.excerpt.length > 0) {
      el += '<span class="excerpt">' + util.truncate(util.stripHTML(list.excerpt), {
        length: 120
      }) + '</span>';
    }
