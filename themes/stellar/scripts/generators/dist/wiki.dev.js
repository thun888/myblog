...
hexo.extend.generator.register('wiki', function (locals) {
  var hasWiki = false;
  locals.pages.forEach(function (page, i) {
    if (page.layout == 'wiki') {
      hasWiki = true;
    }
  });
  ...
  if (hasWiki) {
    ...
    if (hexo.theme.config.wiki && hexo.theme.config.wiki.all_tags) {
      for (var _i = 0, _Object$keys = Object.keys(hexo.theme.config.wiki.all_tags); _i < _Object$keys.length; _i++) {
        var id = _Object$keys[_i];
        var tag = hexo.theme.config.wiki.all_tags[id];
        ret.push({
          path: tag.path,
          data: {
            'filter': true,
