...
hexo.extend.tag.register('friends', function (args) {
  ...
  var _loop = function _loop() {
    ...
    function f() {
      ...
      if (groupId in friends) {
        var group = friends[groupId];

        if (_typeof(group) == 'object' && group.constructor == Array) {
          group = {
            items: group
          };
        }
        ...
        if (group.repo) {
          ...
          group.items.forEach(function (friend, i) {
            el += cell(friend);
          });
          el += '</div>';
        }
  ...
  for (var _i = 0, _Object$keys = Object.keys(friends); _i < _Object$keys.length; _i++) {
    _loop();
  }
